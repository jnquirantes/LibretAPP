...
public class Tema6 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bDownload.setOnClickListener(v -> {
            ...
            PopupMenu popup = new PopupMenu(Tema6.this, v);
                ...
                switch (item.getItemId()) {
                    case (R.id.mGit):
                        if (tabLayout.getTabAt(0).isSelected()) {
                            Uri webpage = Uri.parse("https://acortar.link/jTj6im");
                            Intent webIntent = new Intent(Intent.ACTION_VIEW, webpage);
                            startActivity(webIntent);

                        } else if (tabLayout.getTabAt(1).isSelected()) {
                            Uri webpage = Uri.parse("https://acortar.link/1YGMFl");
                            Intent webIntent = new Intent(Intent.ACTION_VIEW, webpage);
                            startActivity(webIntent);

                        } else {
                            Uri webpage = Uri.parse("https://acortar.link/6SvyIj");
                            Intent webIntent = new Intent(Intent.ACTION_VIEW, webpage);
                            startActivity(webIntent);
                        }
                        break;
